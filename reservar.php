...
$cliente = isClienteLoggedIn() ? getCurrentCliente() : null;
    ...
    <div class="booking-container">
        ...
        <div class="wizard-step" id="step4">
            ...
            <div
                ...
                <div style="margin-bottom: 20px;">
                    ...
                    <input type="text" id="clientName" placeholder="Tu Nombre Completo" value="<?php echo htmlspecialchars($cliente['nombre'] ?? ''); ?>"
                        style="width: 100%; padding: 12px; background: #FFFFFF; border: 1px solid #DDD; color: #555; border-radius: 6px; ">
                </div>

                <div style="margin-bottom: 20px;">
                    <label
                        style="display:block; margin-bottom:8px; color:var(--text-secondary); font-size: 0.9rem;">Email</label>
                    <input type="email" id="clientEmail" placeholder="tu@correo.com (opcional)" value="<?php echo htmlspecialchars($cliente['email'] ?? ''); ?>"
                        style="width: 100%; padding: 12px; background: #FFFFFF; border: 1px solid #DDD; color: #555; border-radius: 6px; ">
                </div>
        ...
        <div class="wizard-step" id="step5">
            ...
            <div style="max-width: 375px; width: calc(100% - 20px); margin: 0 auto; background:#FFFFFF; color:#111111; padding:18px 16px; border-radius:18px; border:1px solid #EAEAEA; box-shadow: 0 10px 30px rgba(0,0,0,0.25); box-sizing: border-box;">
                ...
                <button id="btnConfirmBooking" class="btn btn-next" style="width:100%; background:#111111; color:#FFFFFF; font-size:0.85rem; font-weight: 800; padding: 12px; border:none; border-radius:10px; <?php echo $politicaRequiereCheck ? 'opacity: 0.4; ' : 'opacity: 1; cursor: pointer;'; ?> text-transform: uppercase; letter-spacing: 1px; transition: all 0.2s;" <?php echo $politicaRequiereCheck ? 'disabled' : ''; ?>>
                    CONFIRMAR RESERVA
                </button>
            </div>
        </div>
    ...
    <script>
            ...
            if (currentStep < 5) {
                ...
                if (currentStep === 4) {
                    ...
                    if (!phone || phone.length < 7) {
                        alert('Por favor ingresa un número de WhatsApp / teléfono válido.');
                        return;
                    }
        ...
        document.getElementById('btnConfirmBooking').addEventListener('click', async () => {
            ...
            try {
                ...
                const cName = document.getElementById('clientName') ? document.getElementById('clientName').value.trim() : '';
                const cEmail = document.getElementById('clientEmail') ? document.getElementById('clientEmail').value.trim() : '';
                const cPhone = document.getElementById('clientPhone') ? document.getElementById('clientPhone').value.trim() : '';
                formData.append('telefono', bookingData.phone || cPhone);
                formData.append('cliente_nombre', cName);
                formData.append('cliente_email', cEmail);
        ...
        function updateNavButtons() {
            ...
            if (currentStep === 4) {
                const name = document.getElementById('clientName') ? document.getElementById('clientName').value.trim() : '';
                const phone = document.getElementById('clientPhone') ? document.getElementById('clientPhone').value.trim() : '';
                if (name.length > 1 && phone.length > 6) canNext = true;
            }
